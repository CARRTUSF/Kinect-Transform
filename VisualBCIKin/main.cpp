...
receiving_udpsocket socket1( "0.0.0.0:7001" );
sending_udpsocket socket2( "192.168.1.103:7000" );
...
///This function get the position from the arm chair via sockets
vector<double> getPos(){
	char temp_buf[200];
	string temp_str_buf;
	double x,y,z;
	//send request for position
	output_sock << "GETPOS" << endl;
	//wait for reply
	do{
		getline( read_sock, temp_str_buf );
	} while( temp_str_buf.find("POSITION") == string::npos ); // keep reading until message is received
	sscanf(temp_str_buf.c_str() , "%s %lf %lf %lf",temp_buf, &x, &y, &z);
...
void calculateTransform(std::fstream &pointsFs){
	...
	while(true){
		...
		if (sscanf(temp_buf,"%f,%f,%f,%f,%f,%f", &(KinPos.x), &(KinPos.y), &(KinPos.z), &(ArmPos.x), &(ArmPos.y), &(ArmPos.z)) == 6){
